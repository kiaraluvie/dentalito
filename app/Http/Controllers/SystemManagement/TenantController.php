...
class TenantController extends Controller
{
    ...
    {
        $this->middleware('permission:tenants.view')->only(['index', 'show', 'exportExcel', 'exportPdf', 'exportWord', 'apiKeyManager']);
        $this->middleware('permission:tenants.create')->only(['create', 'store']);
        $this->middleware('permission:tenants.edit')->only(['edit', 'update', 'updateInline', 'generateApiKey']);
        $this->middleware('permission:tenants.delete')->only(['delete', 'deleteSave']);
        $this->middleware('permission:tenants.edit_all')->only('editAll');
    }
    ...
    public function generateApiKey(Tenant $tenant)
    {
        // Find or create a service user for the tenant
        $serviceUser = User::firstOrCreate(
            ...
            [
                'email' => 'service-user-' . $tenant->slug . '@' . config('app.domain', 'localhost'),
                'password' => Hash::make(Str::random(20)),
                'country_id' => 1, // Default country, consider making this configurable
                'locale_id' => 1, // Default locale, consider making this configurable
                'is_active' => true,
            ]
        );
        ...
        // Create a new token
        $token = $serviceUser->createToken('tenant-api-key')->plainTextToken;

        // Save the plain text token to the tenant model
        $tenant->api_token = $token;
        $tenant->save();

        // Redirect back with the token
        return redirect()
            ->route('system_management.tenants.api_key_manager', $tenant->slug)
            ->with('api_key', $token);
    }

    public function apiKeyManager(Tenant $tenant)
    {
        return view('system_management.tenants.api_key_manager', compact('tenant'));
    }
}