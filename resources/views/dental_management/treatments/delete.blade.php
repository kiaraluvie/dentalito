...
@section('title', __('treatments.title_delete'))
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header bg-light border-bottom font-weight-bold">
                    <i class="fas fa-trash"></i> {{ __('treatments.title_delete') }}
                </div>
                <div class="card-body">
                    <p>{{ __('treatments.delete_confirm') }}</p>
                    <form action="{{ route('dental_management.treatments.deleteSave', $treatment) }}" method="POST">
                        @csrf
                        @method('DELETE')
