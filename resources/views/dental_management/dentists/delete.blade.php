    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header bg-danger text-white">
                <h3 class="card-title"><strong><i class="fas fa-trash"></i> {{ __('global.delete') }}: {{ $dentist->user->name }}</strong></h3>
            </div>
            <div class="card-body">
                <p>{{ __('global.are_you_sure') }}</p>
                <p class="text-danger">{{ __('global.irreversible_action') }}</p>
                <form id="form-delete" action="{{ route('dental_management.dentists.deleteSave', $dentist) }}" method="POST">
                    @csrf
                    @method('DELETE')
